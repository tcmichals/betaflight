...
uint32_t generatePWMValue(uint16_t pwmVal)
{
    uint32_t val = pwmVal;

    if ( pwmVal > MAX_ON)
        val = MAX_ON;
    else if ( pwmVal < MIN_ON)
        val = MIN_ON;
                                ...
                                const uint16_t pwmOut_4)
{
	...
	auto txCallback = std::bind(&pwmOutAvalon::txComplete, this,
							    std::placeholders::_1, 
                                std::placeholders::_2);
    uint32_t val = generatePWMValue(pwmOut_1);
    data.push_back(val & 0xFF);
    data.push_back((val >> 8) & 0xFF);
    data.push_back((val >> 16) & 0xFF);
    data.push_back((val >> 24) & 0xFF);

    val = generatePWMValue(pwmOut_2);
    data.push_back(val & 0xFF);
    data.push_back((val >> 8) & 0xFF);
    data.push_back((val >> 16) & 0xFF);
    data.push_back((val >> 24) & 0xFF);

    val = generatePWMValue(pwmOut_3);
    data.push_back(val & 0xFF);
    data.push_back((val >> 8) & 0xFF);
    data.push_back((val >> 16) & 0xFF);
    data.push_back((val >> 24) & 0xFF);

    val = generatePWMValue(pwmOut_4);
    data.push_back(val & 0xFF);
    data.push_back((val >> 8) & 0xFF);
    data.push_back((val >> 16) & 0xFF);
    data.push_back((val >> 24) & 0xFF);    
